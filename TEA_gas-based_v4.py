...
# 수소 및 탄소 생산용량 (kg/일) 및 연료/전력 소비 관련 입력
h2_capacity = st.sidebar.number_input("수소 생산량 (kg/일)", min_value=0.0, value=250.0, step=10.0)
...
# CAPEX 및 OPEX 입력 (250 kg/일 기준 값으로 환산하여 기본값 설정)
capex_construction = st.sidebar.number_input(
    "CAPEX-공사비 (원)",
    min_value=0.0,
    value=1_000_000_000.0 * (h2_capacity / 250.0),
    step=50_000_000.0,
)
capex_equipment = st.sidebar.number_input(
    "CAPEX-설비비 (원)",
    min_value=0.0,
    value=3_000_000_000.0 * (h2_capacity / 250.0),
    step=100_000_000.0,
)
...
opex_annual = st.sidebar.number_input(
    "OPEX-O&M (연간, 원/년)",
    min_value=0.0,
    value=20_000_000.0 * (h2_capacity / 250.0),
    step=1_000_000.0,
)
elec_price = st.sidebar.number_input("전기요금 (원/kWh)", min_value=0.0, value=150.0, step=1.0)
fuel_price = st.sidebar.number_input("연료요금 (원/kg)", min_value=0.0, value=800.0, step=10.0)
h2_price = st.sidebar.number_input("수소 판매가격 (원/kgH₂)", min_value=0.0, value=10000.0, step=100.0)
...
# 연간 수소 및 탄소 생산량 (kg/년)
annual_h2_kg = h2_capacity * (annual_operating_hours / 24.0)
...
annual_fuel_cost = annual_ng_kg * fuel_price
...
# NPV 현금흐름 계산을 위한 리스트 초기화
cash_flows = [-capex_total]  # t=0 초기 투자
pv_in = 0.0
pv_out = capex_total  # 초기 CAPEX의 현재가치(유출)

# 반응기 효율 저하율 및 교체 관련 변수 초기화
reactor_deg_rate = reactor_degradation / 100.0
...
for t in range(1, int(operation_years) + 1):
    ...
    if reactor_replacement_hours > 0 and hours_since_last_replacement >= reactor_replacement_hours:
        opex_replacement_t = opex_annual * 0.3 # 교체 비용 가정: 연간 OPEX의 30%
        hours_since_last_replacement = 0.0
        years_since_last_replacement = 0

    # 효율 저하에 따른 수소생산량, 연료 및 전력 소비량 조정
...
# NPV, B/C, IRR 계산
npv = pv_in - pv_out
bc_ratio = pv_out and (pv_in / pv_out) or np.nan  # 분모 0 방지
try:
...
def compute_lcoh_given_params(elec_price_val, fuel_price_val, capex_total_val, annual_operating_hours_val, reactor_replacement_hours_val, opex_annual_val, discount_rate_val, carbon_price_val):
    ...
    for t in range(1, int(operation_years) + 1):
        ...
        deg_multiplier = (1 - reactor_degradation / 100.0) ** years_since_last_replacement_val if reactor_degradation > 0 else 1.0
...
labels = ["CAPEX-공사", "CAPEX-설비", "OPEX-O&M", "OPEX-전력", "OPEX-연료", "Total LCOH"]
...
for t in range(1, int(operation_years) + 1):
    ...
    # 반응기 효율
    deg_multiplier = (1 - reactor_deg_rate) ** years_since_last_replacement_val if reactor_deg_rate > 0 else 1.0
    ...
    # 세후 현재가치
    numerator_elec += elec_cost_t * (1 - tax_rate) * discount_factor        # 세후 전력요금
    numerator_fuel += fuel_cost_t * (1 - tax_rate) * discount_factor        # 세후 연료요금
    numerator_opex += opex_cost_t * (1 - tax_rate) * discount_factor        # 세후 O&M
    numerator_byprod += - carbon_rev_t * (1 - tax_rate) * discount_factor   # 탄소 수익 (-) 음수값
...
# ===============================================================================================================================================
# 비용 항목 리스트 및 값 (Total 제외) - 전력요금과 연료요금을 분리
drivers = ["CAPEX-공사", "CAPEX-설비", "OPEX-O&M", "OPEX-전력", "OPEX-연료"]
values = [capex_construction_per_kg, capex_equipment_per_kg, opex_per_kg, elec_per_kg, fuel_per_kg]

# 색상 설정 (새 항목 포함하여 색상 지정)
colors = {
    "CAPEX-공사": "#1f77b4",
    "CAPEX-설비": "#aec7e8",
    "OPEX-O&M": "#d62728",
...
# Bar Chart
y_capex_constr = [capex_construction_per_kg, 0, 0, 0, 0, capex_construction_per_kg]
y_capex_equip  = [0, capex_equipment_per_kg, 0, 0, 0, capex_equipment_per_kg]
y_opex         = [0, 0, opex_per_kg, 0, 0, opex_per_kg]
y_elec         = [0, 0, 0, elec_per_kg, 0, elec_per_kg]
y_fuel         = [0, 0, 0, 0, fuel_per_kg, fuel_per_kg]
...
fig_bar.add_bar(name="OPEX-연료", x=labels, y=y_fuel,
               text=[f"{v:,.0f}" if v > 0 else "" for v in y_fuel], textposition="auto", marker_color=colors["OPEX-전력"])
...
with col_lcoh:
    ...
    with col_pie:
        ...
        if total_for_share > 0:
            ...
            st.info("LCOH 계산을 위한 유효한 입력값을 넣어주세요.")
    ...
    with col_sensi:
        ...
        for name in sensi_vars:
            ...
            lcoh_m = compute_lcoh_given_params(
                p["elec_price"],
                p["fuel_price"],
                p["capex_total"],
            ...
            lcoh_p = compute_lcoh_given_params(
                p["elec_price"],
                p["fuel_price"],
                p["capex_total"],                
                p["annual_operating_hours"],
        ...
        fig_sensi.update_layout(
            xaxis_title="ΔLCOH (원/kgH₂)",
            barmode="relative",
            font=dict(family="Arial Black", size=18),
    ...
    fuel_per_kg = fuel_per_kg

    # Total LCOH 막대의 누적 영역 계산
    ...
    x_labels = ["Total LCOH", "CAPEX-공사", "CAPEX-설비", "OPEX-O&M", "OPEX-전력", "OPEX-연료"]
    ...
    fig_major.add_bar(name="OPEX-연료", x=["Total LCOH"], y=[fuel_per_kg], base=[c4], marker_color=colors["OPEX-연료"])
    # 개별 막대
...
with col_fin:
    ...
    fig_fin.update_yaxes(range=[0, max(pv_in, pv_out) * 1.2] if pv_in > 0 and pv_out > 0 else None)
